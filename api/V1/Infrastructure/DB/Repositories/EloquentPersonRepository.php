...
class EloquentPersonRepository implements PersonRepository
{
    ...
    public function filter(array $filters): PersonCollection
    {
        $people = $this->person->orderBy("name");

        foreach ($filters as $field => $value) {
            $people = $people->where($field, $value);
        }

        return $this->makeCollection($people->get());
    }
    ...
    {
        $person = $this->person->create($personData->toArray());
